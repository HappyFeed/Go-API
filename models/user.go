...
func GetUsers() Users {
	...
	for rows.Next() {
		user := User{}
		rows.Scan(&user.Id, &user.Username, &user.Password, &user.Email, &user.createDate)
...
func GetUser(sql string, condicional interface{}) *User {
	user := &User{}
	rows, err := Query(sql, condicional)
	if err != nil {
		return user
	}
